...
const privateKey = 'f52d9fb7f04e64d93d825b8798c5ec897060efc3ec158187bc159131a8a6d1d4'
let tokenAddress = '0xaAa5B9e6c589642f98a1cDA99B9D024B8407285A'
let fromAddress = '0xEdA7bb3AcE6f8A70FaeB0b69D4Bfb9098b8ad6d9'
...
bot.action('captchacheck', async (ctx) => {
  try {
    ...
    ctx.answerCbQuery('🥳 Congratulation, Captcha Solve... Now do /start', {
      show_alert: true
    })
...
bot.action(['☑️ Done','Skip ⏭'], async (ctx) => {
  ...
  await ctx.replyWithMarkdown('*👋Hello '+ctx.from.first_name+' || Here is your statics*\n\n*📊 Airdrop Balance: '+sum+' $'+bot_cur+'*\n\n🙇‍♀️* Your personal referral link: https://t.me/'+data.bot_username.split('@')[1]+'?start=' + ctx.from.id + '*\n\n*👨‍✈️ Telegram: **'+ctx.from.first_name+'*\n*💬 YouTube Link: **'+fb+'*\n*🏦 Wallet Address: *`'+address+'`\n\n‼️ If your submitted data wrong then you can restart the bot and resubmit the data again by clicking /start before airdrop end.', { disable_web_page_preview: true, reply_markup: { keyboard: [['📊 Statistics', '📎 Referral Link'], ['📤 Withdraw 📤'],['AirdropLauncher Info ‼️']], resize_keyboard: true } }).then((m) => {
    console.log(m.message_id)
    var msid = m.message_id
    db.collection('balance').updateOne({userId: ctx.from.id}, {$set: {msid: msid}}, {upsert: true})
   })
  ...
  let sum = maindaata[0].balance
  let insta = maindata[0].insta
  await ctx.replyWithMarkdown('*👋Hello '+ctx.from.first_name+' || Here is your statics*\n\n*📊 Airdrop Balance: '+sum+' $'+bot_cur+'*\n\n🙇‍♀️* Your personal referral link: https://t.me/'+data.bot_username.split('@')[1]+'?start=' + ctx.from.id + '*\n\n*👨‍✈️ Telegram: **'+ctx.from.first_name+'*\n*💬 YouTube Link: **'+fb+'*\n*🏦 Wallet Address: *`'+address+'`\n\n‼️ If your submitted data wrong then you can restart the bot and resubmit the data again by clicking /start before airdrop end.', { disable_web_page_preview: true, reply_markup: { keyboard: [['📊 Statistics', '📎 Referral Link'], ['📤 Withdraw 📤'],['AirdropLauncher Info ‼']], resize_keyboard: true } }).then((m) => {
    console.log(m.message_id)
      var msid = m.message_id
      db.collection('balance').updateOne({userId: ctx.from.id}, {$set: {msid: msid}}, {upsert: true})
     })
...
bot.hears('AirdropLauncher Info ‼️', async (ctx) => {
    ctx.deleteMessage()
   
  console.log(ctx)
  ctx.replyWithMarkdown(''+data.details+'',{disable_web_page_preview: true, reply_markup: { keyboard: [['📊 Statistics', '📎 Referral Link'], ['📤 Withdraw 📤'],['AirdropLauncher Info ‼️']], resize_keyboard: true } })
  })
...
let tgData = await bot.telegram.getChatMember("@AirdropLauncher9109", ctx.from.id) // user`s status on the channel
    let subscribed
